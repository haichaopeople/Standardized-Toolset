...
public class TaskExecutorsTest {
    ...
    public void submit1() {
        ...
        thenSetCount(future);
        Integer integer1 = await().timeout(Duration.FIVE_SECONDS).untilAtomic(EXECUTED_COUNT, equalTo(1));
        log.info("integer1 {}", integer1);
        Integer integer2 = await().timeout(Duration.ONE_SECOND).untilAtomic(COMPLETE_COUNT, equalTo(1));
        log.info("integer2 {}", integer2);
        Integer integer3 = await().timeout(Duration.ONE_SECOND).untilAtomic(SUCCESS_COUNT, equalTo(1));
        log.info("integer3 {}", integer3);
        Integer integer4 = await().timeout(Duration.ONE_SECOND).untilAtomic(FAILURE_COUNT, equalTo(0));
        log.info("integer4 {}", integer4);
        log.info("EXECUTED_COUNT.get() {}", EXECUTED_COUNT.get());
    ...
    public void submits2() {
        ...
        Integer integer1 = await().timeout(Duration.TWO_MINUTES).untilAtomic(EXECUTED_COUNT, equalTo(CYCLE));
        log.info("integer1 {}", integer1);
        Integer integer2 = await().timeout(Duration.ONE_SECOND).untilAtomic(COMPLETE_COUNT, equalTo(CYCLE));
        log.info("integer2 {}", integer1);
        Integer integer3 = await().timeout(Duration.ONE_SECOND).untilAtomic(SUCCESS_COUNT, equalTo(CYCLE));
        log.info("integer3 {}", integer1);
        Integer integer4 = await().timeout(Duration.ONE_SECOND).untilAtomic(FAILURE_COUNT, equalTo(0));
        log.info("integer4 {}", integer1);
